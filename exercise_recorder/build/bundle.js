...
function handleArrayEvents(submission, eventData) {
  switch(eventData.type) {
    case 'jsav-array-click':
      submission.animation.push({
        type: eventData.type,
        tstamp: eventData.tstamp,
        currentStep: eventData.currentStep,
        data: {
          arrayid: eventData.arrayid,
...
function handleStateChange(submission, exercise, eventData) {
  // Filter to remove undefined elements
  const newStates = stateChanged(submission, exercise).filter(s => 
    s && Object.keys(s).length > 0);
  if(newStates.length) {
    addNewStatesToSubmission(submission, eventData, newStates);
    return newStates;
  }  
  return false;
}

function handleGradeButtonClick(submission, eventData) {
  submission.animation.push({
    type: eventData.type,
    tstamp: eventData.tstamp,
    currentStep: eventData.currentStep,
    data: { score: { ...eventData.score } }
  });
}

function addNewStatesToSubmission(submission, eventData, newStates) {
  const currentStep = eventData.currentStep !== undefined ? eventData.currentStep
    : submission.animation[submission.animation.length - 1].currentStep +1;
  newStates.forEach((state, i) => {
  submission.animation.push({
    type: eventData.type || 'not specified',
    tstamp: eventData.tstamp || new Date(),
    currentStep,
    data: { dataStructureId: state.id, state: state.values }
  });
});
}

function stateChanged(submission, exercise) {  
  const dataStructures = submission.initialState.map( ds => {
    return {
      type: ds.type,
      id: ds.id,
      values: ds.values
  ...
  return getNewStates(dataStructures, exercise);
}
...
function setExerciseOptions(submission, eventData) {
  submission.definitions.options = {
...
function setFinalGrade(subsmission, eventData) {
  subsmission.definitions.score = { ...eventData.score };
}
...
},{"../utils/helperFunctions.js":34}],3:[function(require,module,exports){
"use strict";
...
"use strict";
const metad_func = require('./metadata/metadata.js');
const def_func = require('./definitions/definitions.js');
const init_state_func = require('./initialState/initialState.js');
const anim_func = require('./animation/animation.js');
const services = require('./rest/services.js');
...
function passEvent(eventData) {
  ...
  switch(eventData.type){
    case 'jsav-init':
      def_func.setExerciseOptions(submission, eventData);
      metad_func.setExerciseMetadata(submission, eventData);
      break;
    case 'jsav-exercise-init':
      exercise = eventData.exercise;
      jsav = exercise.jsav;
      def_func.setDefinitions(exercise);
      init_state_func.setInitialDataStructures(submission, exercise,passEvent);
      break;
      // Here we handle all array related events
    case String(eventData.type.match(/^jsav-array-.*/)):
      anim_func.handleArrayEvents(submission, eventData);
      break;
    case 'recorder-set-id':
      init_state_func.setNewId(submission, eventData);
      break;
    case 'jsav-exercise-gradeable-step':
      anim_func.handleStateChange(submission, exercise, eventData);
      break;
    case 'jsav-exercise-grade-button':
      anim_func.handleGradeButtonClick(submission, eventData);
      break;
    case 'jsav-exercise-grade':
      def_func.setFinalGrade(submission, eventData);
      services.sendSubmission(submission);
      break;
    default:
...
// TODO: pass values instead of reference
const getSubmissionState = () => submission;
...
},{"./animation/animation.js":1,"./definitions/definitions.js":2,"./initialState/initialState.js":5,"./metadata/metadata.js":6,"./rest/services.js":33}],5:[function(require,module,exports){
"use strict";
const error_handler = require('../errors/errors.js');
const recorder = require('../exerciseRecorder.js');

function setInitialDataStructures(submission, exercise) {
  ...
  dataStructures.forEach(dataStructure => submission.initialState.push(dataStructure));
}
...
function setNewId(submission, eventData) {
  const dsIndex = submission.initialState.findIndex(ds => ds.id === eventData.tempId);
  submission.initialState[dsIndex].id = eventData.newId;
}
...
},{"../errors/errors.js":3,"../exerciseRecorder.js":4}],6:[function(require,module,exports){
"use strict";
...
},{"./adapters/http":8,"./adapters/xhr":8,"./helpers/normalizeHeaderName":29,"./utils":32,"_process":35}],23:[function(require,module,exports){
'use strict';
